    ...
    server.listen(port, function () {
        function respondTo(request, response, next) {
            ...
            try {
                ...
                if (stub) {
                    var contentType = response.header('content-type') || stub.response.contentType;

                    response.header('Content-Type', contentType);

                    if (stub.response.headers) {
                        _.keys(stub.response.headers).forEach(function (header) {
        ...
        server.get(/.*/, function (request, response, next) {
            function isRequestForStaticContent(request) {
                if (staticServer) {
                    ...
                    return ((staticServer.include.length === 0) || (staticServer.include.some(matchesUrl))) &&
