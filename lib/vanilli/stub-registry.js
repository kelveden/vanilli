...
exports.create = function (config) {
    function StubRegistry (appLog) {
                ...
                if (stub.criteria.query) {
                    _.keys(stub.criteria.query)
                        .forEach(function (queryParam) {
                            ...
                            } else if (_.isArray(queryCriteria)) {
                                stub.criteria.query[ queryParam ] = queryCriteria.map(function (item) {
        ...
        this.addExpectation = function (stub) {
            var stubToExpectation = R.partial(R.merge, [{ expect: true, times: 1 }]);

            return self.addStub(stubToExpectation(stub));
        };
        ...
        this.findMatchFor = function (request) {
            ...
            function stringAsObject (thing) {
                return (typeof thing === 'string') ? JSON.parse(thing) : thing;
            }

            function objectContains (container, containee) {
                if (_.isObject(containee)) {
                    return _.keys(containee).reduce(function (acc, key) {
                        var shareProp = container.hasOwnProperty(key);
                        if (!shareProp) {
                            log.debug("Property was not shared: " + key);
                        }
                        return acc && shareProp && objectContains(container[key], containee[key]);
                    }, true);
                } else {
            ...
            function criteriaMatchesPrimitive (criteria, value) {
                return (typeof value !== 'undefined') &&
                    ...
                    (_.isArray(criteria) && (_.isEmpty(_.difference(criteria, value))));
            }
            ...
            function bodiesMatch (request, stub) {
                if (stub.criteria.body) {
                    ...
                    } else if (!objectContains(stringAsObject(bufferAsString(request.body)), stringAsObject(stub.criteria.body))) {
                        log.debug("REJECTED stub '" + stub.id + "' on body [Expected: '" + objectAsString(stub.criteria.body) + "'; Actual: " + objectAsString(request.body) + "]");
                        return false;
