...
exports.start = function (config) {
    ...
    server.listen(port, function () {
        function respondTo(request, response, next) {
            function respondWithStub (request) {
            ...
            function respondWithStatic (staticPath) {
                response.send(200, fs.readFile(staticPath));
            }

            try {
                if (staticRoot) {
                    var staticPath = path.join(".", staticRoot, request.getPath());

                    fs.exists(staticPath, function (exists) {
                        if (exists) {
                            respondWithStatic(staticPath);
                        } else {
                            respondWithStub(request);
                        }
                    });
                } else {
                    respondWithStub(request);
