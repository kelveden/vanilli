...
exports.start = function (config) {
    var port = config.port,
        ...
        staticFilter = [], serveStaticFunc;

    if (config.static) {
        serveStaticFunc = restify.serveStatic({
            directory: config.static.root,
            default: 'index.html'
        });
        staticFilter = config.static.filter;
    }
    ...
    server.listen(port, function () {
        ...
        server.get(/.*/, function (request, response, next) {
            if (staticFilter.some(_.partial(minimatch, request.url))) {
                serveStaticFunc(request, response, next);
