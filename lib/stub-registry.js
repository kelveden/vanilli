...
exports.create = function (appLog) {
    function StubRegistry(appLog) {
        ...
        this.findMatchFor = function (request) {
            var requestUrl = ((request.url.substr(0, 1) !== "/") ? "/" : "") + request.url;

            log.debug("Finding match for request: " + requestUrl);

            var matchingStubs = registry.filter(function (stub) {
                if (!requestUrl.match(stub.criteria.url)) {
                    log.debug("REJECTED on url [Expected: " + stub.criteria.url + "; Actual: " + requestUrl + "]");
                    return false;
                }
