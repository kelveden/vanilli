    ...
    function StubRegistry(appLog) {
        ...
        this.findMatchFor = function (request) {
            function bodyMatches (expected, actual) {
                ...
                return (expected.regex && objectAsString(actual).match(expected.regex)) || (objectAsString(expected) === objectAsString(actual));
            }
            ...
            var matchingStubs = registry.filter(function (stub) {
                var criteriaUrl = stub.criteria.url;

                if (criteriaUrl.substr(0, 1) !== "^") {
                    criteriaUrl = "^" + criteriaUrl;
                }
                if (criteriaUrl.substr(criteriaUrl.length - 1, 1) !== "$") {
                    criteriaUrl = criteriaUrl + "$";
                }

                if (!requestUrl.match(criteriaUrl)) {
                ...
                if (stub.criteria.body) {
                    ...
                    } else if (!bodyMatches(stub.criteria.body, request.body)) {
                        log.debug("REJECTED stub '" + stub.id + "' on body [Expected: '" + stub.criteria.body + "'; Actual: " + request.body + "]");
                        return false;
